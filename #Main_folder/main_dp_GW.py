...
Development:
    # Data preprocessing: #
        ...
        ** Result **
            The result of this phase will be a dataframe for each selected SSA
            component of each water table stations point. It will contain the
            water table depth time series, the precipitation time series and
            the evaporation time series.
...
pkl.dump(df_log, open('loggers_dataframe.p', 'wb'))
pkl.dump(log_coord, open('log_coordinates.p', 'wb'))

#To load them:
# df_log = pkl.load(open('loggers_dataframe.p', 'rb'))
# log_coord = pkl.load(open('log_coordinates.p', 'rb'))

# %% Data Preprocessing: Select the nearest Weather Stations to the Water Table Depth stations

# Load the weather stations
path = r'D:\Users\colompa\Documents\KWR_Internship\Data\Weather_data\metadata.pkl'
with open(path, 'rb') as handle:
            md_weather, _, _, _, _= pkl.load(handle)
path = r'D:\Users\colompa\Documents\KWR_Internship\Data\Weather_data\data.pkl'
with open(path, 'rb') as handle:
...
prec_coord.index = range(0, len(prec_coord.index))
...
evap_coord.index = range(0, len(evap_coord.index))

# Select the closest weather station to each GW station
log_coord.rename(columns = {log_coord.columns[0]: 'Lat', log_coord.columns[1]: 'Lon'}, inplace = True)
...
# %% Export Nearest stations data and identification

pkl.dump(df_prec_cut, open('df_prec.p', 'wb'))
pkl.dump(df_evap_cut, open('df_evap.p', 'wb'))

pkl.dump(nearest_stations_p, open('near_prec.p', 'wb'))
pkl.dump(nearest_stations_e, open('near_evap.p', 'wb'))
...
for i in range(len(df_log.columns)):
    logg = df_log.iloc[:,i][df_log.iloc[:,i].notna()]
...
#Save the result
pkl.dump(dSSA, open('SSA_components.p', 'wb'))
# To load: pkl.load(open('SSA_components.p', 'rb'))

#Load the information about the selected SSA components for each station
SSAinfo = pkl.load(open('SSA_information.p', 'rb'))
