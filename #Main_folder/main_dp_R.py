...
#The predicted variable will be the water level, while the flow will be used
#as a feature since it is located upstream
keep = [0, 1, 2, 3, 6, 7, 8]
df_river = df_river.iloc[:,keep]
df_river.columns = ['level', 'flow', 'prec', 'evap', 'extr1', 'extr2', 'extr3']
...
dp.interactive_df_visualization(df_river)
...
# %% SSA on the water level

#SSA components
L = 365
process = df_river.iloc[:, 0][df_river.iloc[:, 0].notna()]
SSA_flow = SSA(process, L)
pkl.dump(SSA_flow, open('SSA_level.p', 'wb'))

#Obtain trend, periodicities and noise
dp.plot_Wcorr_Wzomm(SSA_flow, 'River level')
dp.plot_Wcorr_Wzomm(SSA_flow, 'River level', 49)
dp.plot_Wcorr_Wzomm(SSA_flow, 'River level', 9)
...
dp.plot_SSA_results(SSA_flow, Fs, label = 'River level - SSA', file = f'{path_plot}\\river_SSA.html')
...
SSA_flow_info = components_SSA(0, 'level', F0, F1, F2, 5, 365)
pkl.dump(SSA_flow_info, open('SSA_level_info.p', 'wb'))
